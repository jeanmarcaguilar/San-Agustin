...
// Recommendations page - view only (borrow from Available Books page)

// Get search parameter
$search = $_GET['search'] ?? '';

// Build query to get books from librarian database (simplified - no recommendations table yet)
$query = "
    SELECT b.*
    FROM books b
    WHERE b.available > 0
...
<head>
  ...
  <title>📚 Recommendations – San Agustin Elementary School</title>
  ...
  <style>
    ...
    .recommendation-badge.system { background-color: #e0f2fe; color: #0369a1; }
    .recommendation-badge.librarian { background-color: #f0fdf4; color: #166534; }
    .recommendation-badge.teacher { background-color: #fef9c3; color: #854d0e; }
    ...
    .score-meter-fill {
      height: 100%;
      background: linear-gradient(90deg, #60a5fa, #0b6b4f);
      border-radius: 3px;
    }
      ...
      <nav class="mt-3 flex-1 space-y-2">
        ...
        <div class="collapsible-section">
          ...
          <div class="collapsible-content bg-green-900 rounded-lg mt-1 overflow-hidden">
            ...
            <a href="recommendations.php" class="block px-4 py-2 text-white hover:bg-green-700 transition-colors active-nav">
    ...
    <main class="flex-1 p-5">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="relative">
            <form method="GET" class="flex items-center">
              ...
              <?php if (!empty($search)): ?>
                <a href="recommendations.php" class="ml-2 p-2 text-gray-500 hover:text-gray-700">
      ...
      <section class="card p-5">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-bold text-school-primary flex items-center gap-2">
            <i class="fas fa-star"></i> Recommended Books
          </h2>
          <div class="text-sm text-gray-600">
            <?php echo count($recommendedBooks); ?> book(s) recommended for you
          </div>
        </div>
        ...
        <?php if (empty($recommendedBooks)): ?>
          <div class="text-center py-12">
            ...
            <p class="text-gray-700 font-medium">No recommendations available at the moment</p>
            <p class="text-gray-500 mt-1">Check back later for personalized book recommendations.</p>
            <a href="available-books.php" class="inline-block mt-4 text-school-accent hover:underline">
              <i class="fas fa-arrow-right mr-1"></i> Browse all available books
            </a>
          </div>
        <?php else: ?>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php foreach ($recommendedBooks as $book): 
              $recommendationSource = $book['recommended_by'] ?? 'system';
              $sourceIcons = [
                'system' => 'fa-robot',
              ...
              $sourceLabels = [
                ...
                'librarian' => 'Recommended by Librarian',
                'teacher' => 'Recommended by Teacher'
              ];
              $scorePercent = min(100, max(0, ($book['score'] ?? 0.5) * 100));
            ?>
              <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                <div class="p-5">
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                      <h3 class="font-bold text-lg text-gray-900 mb-1"><?php echo htmlspecialchars($book['title']); ?></h3>
                      <p class="text-sm text-gray-600"><?php echo htmlspecialchars($book['author']); ?></p>
                    </div>
                    <div class="w-20 h-28 bg-gradient-to-br from-blue-100 to-purple-100 rounded overflow-hidden flex-shrink-0 ml-3">
                      <?php 
                  ...
                  <div class="mt-2">
                    <div class="flex items-center justify-between text-xs text-gray-500 mb-1">
                      ...
                      <span><?php echo $book['available']; ?> available</span>
                    </div>
                    
                    <div class="mt-3 text-sm text-gray-700">
                      <div class="flex items-center justify-between mb-1">
                        <span class="font-medium">Match Score</span>
                        <span class="font-semibold"><?php echo number_format($scorePercent, 0); ?>%</span>
                      </div>
                      <div class="score-meter">
                        <div class="score-meter-fill" style="width: <?php echo $scorePercent; ?>%"></div>
                      </div>
                    </div>
                    
                    <?php if (!empty($book['reason'])): ?>
                      <div class="mt-3 p-2 bg-gray-50 rounded text-xs text-gray-600">
                        <i class="fas fa-info-circle mr-1 text-school-accent"></i>
                        <?php echo htmlspecialchars($book['reason']); ?>
                      </div>
                    <?php endif; ?>
                    
                    <div class="mt-4 flex items-center justify-between">
                      <span class="recommendation-badge <?php echo $recommendationSource; ?>">
                        <i class="fas <?php echo $sourceIcons[$recommendationSource] ?? 'fa-star'; ?>"></i>
                        <?php echo $sourceLabels[$recommendationSource] ?? 'Recommended'; ?>
                      </span>
                      
                      <a 
                        href="available-books.php?search=<?php echo urlencode($book['title']); ?>" 
                        class="big-btn bg-school-accent hover:bg-blue-500 text-white flex items-center gap-2"
                      >
      ...
      <footer class="mt-8 text-center border-t pt-6 pb-4 bg-white rounded-xl shadow-sm">
        ...
        <div class="text-sm text-gray-500 border-t pt-3">
          &copy; <?php echo date('Y'); ?> San Agustin Elementary School Student Portal • Learning is Fun!
        </div>
      </footer>
    </main>
  </div>
