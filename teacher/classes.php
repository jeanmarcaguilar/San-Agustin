...
try {
    ...
    $error_messages[] = "An error occurred while loading the classes page: " . $e->getMessage();
    $user = [
...
<head>
    ...
    <title>San Agustin Elementary School - My Classes</title>
...
<body class="min-h-screen flex">
    ...
    <div id="sidebar" class="sidebar w-64 min-h-screen flex flex-col text-white">
        ...
        <div class="flex-1 p-4 overflow-y-auto custom-scrollbar">
            <ul class="space-y-2">
                ...
                <li>
                    <a href="classes.php" class="flex items-center p-3 rounded-lg bg-secondary-700 text-white transition-colors nav-item">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <header class="header-bg text-white p-4 flex items-center justify-between shadow-md">
            <div class="flex items-center">
                ...
                <h1 class="text-xl font-bold">My Classes</h1>
            </div>
                ...
                <div class="bg-white rounded-xl p-6 mb-6 shadow-sm border border-gray-200 dashboard-card">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="mt-4 md:mt-0">
                            <button type="button" onclick="addNewClass()" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary-600 hover:bg-primary-700 focus:outline-none">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 dashboard-card">
                    <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                        <div class="flex items-center">
                            ...
                            <h3 class="text-base font-semibold text-gray-800">Class List</h3>
                            <span class="ml-2 text-sm text-gray-500"><?php echo count($classes); ?> classes</span>
                        </div>
                        <div class="relative">
                            <input type="text" id="search-classes" placeholder="Search classes..." class="pl-10 pr-4 py-2 border border-gray-300 rounded-md text-sm focus:ring-primary-500 focus:border-primary-500">
                    ...
                    <div class="flex-1 overflow-y-auto" style="max-height: 600px;">
                                ...
                                <?php foreach ($classes as $class): ?>
                                    <li class="px-4 py-4 hover:bg-gray-50 transition-colors duration-150">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 bg-primary-100 p-2 rounded-md">
                                                    <i class="fas fa-chalkboard-teacher text-primary-600 text-sm"></i>
                                                </div>
                                                <div class="ml-3 flex-1 min-w-0">
                                                    <p class="text-sm font-medium text-gray-900 truncate">
                                                        <?php echo htmlspecialchars($class['subject']); ?>
                                                        <?php if (!empty($class['grade_level']) && !empty($class['section'])): ?>
                                                            <span class="text-gray-500">• Grade <?php echo htmlspecialchars($class['grade_level'] . ' - ' . $class['section']); ?></span>
                                                        <?php endif; ?>
                                                    </p>
                                            ...
                                            <div class="flex items-center space-x-2">
                                                <button onclick="viewClass('<?php echo $class['id']; ?>', '<?php echo htmlspecialchars($class['subject']); ?>', '<?php echo $class['grade_level']; ?>', '<?php echo htmlspecialchars($class['section']); ?>', '<?php echo htmlspecialchars($class['schedule'] ?? ''); ?>', '<?php echo htmlspecialchars($class['room'] ?? ''); ?>')" class="text-xs text-primary-600 hover:text-primary-800 focus:outline-none">
                                                    <i class="fas fa-eye mr-1"></i>View
                                                </button>
                                                <button onclick="editClass('<?php echo $class['id']; ?>', '<?php echo htmlspecialchars($class['subject']); ?>', '<?php echo $class['grade_level']; ?>', '<?php echo htmlspecialchars($class['section']); ?>', '<?php echo htmlspecialchars($class['schedule'] ?? ''); ?>', '<?php echo htmlspecialchars($class['room'] ?? ''); ?>')" class="text-xs text-gray-600 hover:text-gray-800 focus:outline-none">
                        ...
                        <?php else: ?>
                            <div class="p-4 text-center">
                                <i class="fas fa-chalkboard-teacher text-3xl text-gray-200 mb-2"></i>
                                <h3 class="text-sm font-medium text-gray-900">No classes found</h3>
                                <p class="mt-1 text-xs text-gray-500">Click "Add New Class" to get started</p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <!-- View Class Modal -->
    <div id="viewClassModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg w-full max-w-md mx-4">
            <div class="p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-semibold text-gray-800" id="viewClassName"></h3>
                    <button type="button" onclick="closeModal('viewClassModal')" class="text-gray-400 hover:text-gray-500">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p id="viewClassSubject" class="font-medium"></p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Grade Level</p>
                        <p id="viewClassGrade" class="font-medium"></p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Section</p>
                        <p id="viewClassSection" class="font-medium"></p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Schedule</p>
                        <p id="viewClassSchedule" class="font-medium"></p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Room</p>
                        <p id="viewClassRoom" class="font-medium"></p>
                    </div>
                </div>
                <div class="mt-6 flex justify-end space-x-3">
                    <button type="button" onclick="closeModal('viewClassModal')" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                        Close
                    </button>
                </div>
            </div>
    ...
    <div id="editClassModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg w-full max-w-md mx-4">
            <form id="classForm" action="save_class.php" method="POST">
                ...
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-xl font-semibold text-gray-800" id="modalTitle">Add New Class</h3>
                        <button type="button" onclick="closeModal('editClassModal')" class="text-gray-400 hover:text-gray-500">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <div class="space-y-4">
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        <button type="button" onclick="closeModal('editClassModal')" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Cancel
                        </button>
    ...
    <script>
        ...
        // View class details
        function viewClass(id, subject, gradeLevel, section, schedule, room) {
            document.getElementById('viewClassName').textContent = `${subject} - Grade ${gradeLevel} ${section}`;
            document.getElementById('viewClassSubject').textContent = subject;
            document.getElementById('viewClassGrade').textContent = `Grade ${gradeLevel}`;
            document.getElementById('viewClassSection').textContent = section;
            document.getElementById('viewClassSchedule').textContent = schedule || 'Not specified';
            document.getElementById('viewClassRoom').textContent = room || 'Not specified';
            openModal('viewClassModal');
        }

        // Edit class
        function editClass(id, subject, gradeLevel, section, schedule, room) {
        ...
        document.getElementById('classForm').addEventListener('submit', async function(e) {
            ...
            const response = await fetch('save_class.php', {
                method: 'POST',
                body: formData
            });
            ...
            if (result.success) {
                ...
                alert('Error: ' + (result.message || 'Failed to save class'));
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('San Agustin Elementary School Classes Page loaded');
            ...
            const classItems = document.querySelectorAll('ul.divide-y > li');
