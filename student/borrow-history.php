...
if ($patron) {
    // Build query to get borrow history with book details from librarian database
    $query = "
        SELECT t.*, b.title, b.author, b.isbn, b.publisher, b.cover_image
        FROM transactions t
...
<head>
  ...
  <title>📜 Borrow History – San Agustin Elementary School</title>
...
<body class="min-h-screen bg-gray-50">
  <div class="min-h-screen flex">
    <aside class="hidden md:flex md:flex-col w-72 p-5 bg-school-primary text-white">
      ...
      <nav class="mt-3 flex-1 space-y-2">
        ...
        <div class="collapsible-section">
          ...
          <div class="collapsible-content bg-green-900 rounded-lg mt-1 overflow-hidden">
            ...
            <a href="borrow-history.php" class="block px-4 py-2 text-white hover:bg-green-700 transition-colors active-nav">
      ...
      <div class="mt-4">
        <a href="../login.php" class="w-full bg-red-600 hover:bg-red-500 big-btn flex items-center justify-center gap-2 px-3 py-3 rounded">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="relative">
            <form method="GET" class="flex items-center">
              ...
              <?php if (!empty($search)): ?>
                <a href="borrow-history.php" class="ml-2 p-2 text-gray-500 hover:text-gray-700">
      ...
      <section class="card p-5">
        <div class="flex justify-between items-center mb-6">
          ...
          <div class="text-sm text-gray-600">
            <?php echo count($borrowHistory); ?> record(s) found
          </div>
        </div>
        ...
        <?php if (empty($borrowHistory)): ?>
          <div class="text-center py-12">
            ...
            <?php if (!empty($search)): ?>
              <a href="borrow-history.php" class="text-school-accent hover:underline mt-2 inline-block">
                Clear search
              </a>
            <?php endif; ?>
          </div>
            ...
            <table class="w-full text-sm text-left text-gray-700">
              <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                <tr>
                  ...
                  <th scope="col" class="px-6 py-3">Returned On</th>
                  <th scope="col" class="px-6 py-3 text-center">Status</th>
                </tr>
              </thead>
              <tbody>
                <?php foreach ($borrowHistory as $loan): ?>
                  <tr class="bg-white border-b hover:bg-gray-50">
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        <div class="w-12 h-16 bg-gradient-to-br from-blue-100 to-purple-100 rounded overflow-hidden flex-shrink-0">
                          <?php 
                            $hasImage = !empty($loan['cover_image']) && file_exists("../Uploads/books/" . $loan['cover_image']);
                          ?>
                          <?php if ($hasImage): ?>
                            <img 
                              src="../Uploads/books/<?php echo htmlspecialchars($loan['cover_image']); ?>" 
                        ...
                        <div>
                          ...
                          <div class="text-xs text-gray-500"><?php echo htmlspecialchars($loan['author']); ?></div>
                        </div>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                      <div class="font-medium"><?php echo date('M d, Y', strtotime($loan['checkout_date'])); ?></div>
                      <div class="text-xs text-gray-500"><?php echo date('h:i A', strtotime($loan['checkout_date'])); ?></div>
                    </td>
                    <td class="px-6 py-4">
                      <div class="font-medium"><?php echo date('M d, Y', strtotime($loan['due_date'])); ?></div>
                      <?php 
                        $dueDate = new DateTime($loan['due_date']);
                        $today = new DateTime();
                        $isOverdue = $dueDate < $today && $loan['status'] !== 'returned';
                        if ($isOverdue): 
                      ?>
                    ...
                    <td class="px-6 py-4">
                      <?php if ($loan['return_date']): ?>
                        <div class="font-medium"><?php echo date('M d, Y', strtotime($loan['return_date'])); ?></div>
                        <div class="text-xs text-gray-500"><?php echo date('h:i A', strtotime($loan['return_date'])); ?></div>
                      <?php else: ?>
                    ...
                    <td class="px-6 py-4 text-center">
                      <?php 
                        ...
                        if ($isOverdue) {
                          ...
                          $statusText = 'Returned';
                        } elseif ($loan['status'] === 'checked_out') {
