...
try {
    switch ($report_type) {
        case 'by_grade':
            $report_title = 'Enrollment by Grade Level';
        ...
        case 'by_section':
            $report_title = 'Enrollment by Section';
            $query = "SELECT CONCAT(grade_level, '-', section) as class, COUNT(*) as count 
                     FROM students 
                     WHERE status = 'Active' 
                     AND school_year = ? 
                     " . ($grade_level ? " AND grade_level = " . (int)$grade_level : '') . "
        ...
        case 'by_gender':
            $report_title = 'Enrollment by Gender';
            $query = "SELECT gender, COUNT(*) as count 
                     FROM students 
                     WHERE status = 'Active' 
                     AND school_year = ? 
                     " . ($grade_level ? " AND grade_level = " . (int)$grade_level : '') . "
                     " . ($section ? " AND section = " . $pdo->quote($section) : '') . "
                     GROUP BY gender 
                     ORDER BY gender";
            $stmt = $pdo->prepare($query);
            $stmt->execute([$selected_year]);
        ...
        default:
            $report_type = 'summary';
            $report_title = 'Enrollment Summary';
            
            $query = "SELECT COUNT(*) as total FROM students WHERE status = 'Active' AND school_year = ?";
            $stmt = $pdo->prepare($query);
            $stmt->execute([$selected_year]);
            ...
            $stmt = $pdo->prepare($query);
            $stmt->execute([$selected_year]);
            $by_grade = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            $stmt = $pdo->prepare($query);
            $stmt->execute([$selected_year]);
            $by_gender = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            $stmt = $pdo->prepare($query);
            $stmt->execute([$selected_year]);
            $top_sections = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
switch ($report_type) {
    case 'by_grade':
        foreach ($report_data as $row) {
            $chart_data['labels'][] = 'Grade ' . $row['grade_level'];
    ...
    case 'summary':
    default:
        foreach ($report_data['by_grade'] as $row) {
            $chart_data['labels'][] = 'Grade ' . $row['grade_level'];
            $chart_data['datasets'][0]['data'][] = $row['count'];
        }
        $chart_type = 'bar';
        ...
        <div class="container mx-auto px-4 py-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                <div>
                    ...
                    <p class="text-gray-600">View and analyze student enrollment data</p>
                </div>
                ...
                <form action="" method="get" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select id="report_type" name="report_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="by_grade" <?= $report_type == 'by_grade' ? 'selected' : '' ?>>By Grade Level</option>
                            <option value="by_section" <?= $report_type == 'by_section' ? 'selected' : '' ?>>By Section</option>
                            <option value="by_gender" <?= $report_type == 'by_gender' ? 'selected' : '' ?>>By Gender</option>
                        </select>
                    </div>
                    
                    <div id="gradeLevelFilter" style="display: <?= in_array($report_type, ['by_section', 'by_gender']) ? 'block' : 'none' ?>">
                    ...
                    <div id="sectionFilter" style="display: <?= $report_type == 'by_gender' ? 'block' : 'none' ?>">
                        ...
                        <select id="section" name="section" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <?php foreach ($sections as $sec): ?>
                                <?php if (!$grade_level || $sec['grade_level'] == $grade_level): ?>
                                    <option value="<?= $sec['section'] ?>" <?= $section == $sec['section'] ? 'selected' : '' ?>>
                                        Grade <?= $sec['grade_level'] ?> - <?= $sec['section'] ?>
                                    </option>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </select>
            ...
            <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                ...
                <?php if ($report_type === 'summary'): ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div class="report-card bg-white border border-gray-200 rounded-lg p-6 text-center">
                            <div class="text-4xl font-bold text-blue-600 mb-2"><?= number_format($report_data['total']) ?></div>
                            <div class="text-sm font-medium text-gray-500">Total Students</div>
                        </div>
                        
                        <div class="report-card bg-white border border-gray-200 rounded-lg p-6">
                            <h3 class="text-lg font-medium text-gray-800 mb-4">By Grade Level</h3>
                            <div class="space-y-3">
                                <?php foreach ($report_data['by_grade'] as $grade): ?>
                                    <div>
                                        <div class="flex justify-between text-sm mb-1">
                                            <span class="font-medium">Grade <?= $grade['grade_level'] ?></span>
                                            <span class="text-gray-600"><?= $grade['count'] ?> students</span>
                                        </div>
                                        <div class="w-full bg-gray-200 rounded-full h-2">
                                            <div class="bg-blue-600 h-2 rounded-full" style="width: <?= ($grade['count'] / $total_students) * 100 ?>%"></div>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        ...
                        <div class="report-card bg-white border border-gray-200 rounded-lg p-6">
                            <h3 class="text-lg font-medium text-gray-800 mb-4">By Gender</h3>
                            <div class="space-y-3">
                                <?php 
                                $male = 0;
                                $female = 0;
                                ...
                                foreach ($report_data['by_gender'] as $gender) {
                                    if ($gender['gender'] === 'Male') $male = $gender['count'];
                                    elseif ($gender['gender'] === 'Female') $female = $gender['count'];
                                    else $other += $gender['count'];
                                }
                                
                                $gender_data = [
                                    ['label' => 'Male', 'count' => $male, 'color' => 'blue-600'],
                                    ['label' => 'Female', 'count' => $female, 'color' => 'pink-500'],
                                    ['label' => 'Other', 'count' => $other, 'color' => 'gray-400']
                                ];
                                ?>
                                
                                <?php foreach ($gender_data as $item): ?>
                                    <?php if ($item['count'] > 0): ?>
                                        <div>
                                            <div class="flex justify-between text-sm mb-1">
                                                <span class="font-medium"><?= $item['label'] ?></span>
                                                <span class="text-gray-600"><?= $item['count'] ?> (<?= $total_students > 0 ? round(($item['count'] / $total_students) * 100, 1) : 0 ?>%)</span>
                                            </div>
                                            <div class="w-full bg-gray-200 rounded-full h-2">
                                                <div class="h-2 rounded-full bg-<?= $item['color'] ?>" style="width: <?= $total_students > 0 ? ($item['count'] / $total_students) * 100 : 0 ?>%"></div>
                                            </div>
                                        </div>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="report-card bg-white border border-gray-200 rounded-lg p-6">
                            <h3 class="text-lg font-medium text-gray-800 mb-4">Enrollment by Grade Level</h3>
                        ...
                        <div class="report-card bg-white border border-gray-200 rounded-lg p-6">
                            <h3 class="text-lg font-medium text-gray-800 mb-4">Top 5 Largest Sections</h3>
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <?php foreach ($report_data['top_sections'] as $section): ?>
                                            <tr>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?= $section['class'] ?></td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right"><?= $section['count'] ?></td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                                                    <?= $total_students > 0 ? round(($section['count'] / $total_students) * 100, 1) : 0 ?>%
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                ...
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            <?= $report_type === 'by_gender' ? 'Gender' : ($report_type === 'by_grade' ? 'Grade Level' : 'Class') ?>
                                        </th>
                                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Number of Students</th>
                                    ...
                                    <?php foreach ($report_data as $row): ?>
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                <?= $report_type === 'by_gender' ? ($row['gender'] ?: 'Not Specified') : 
                                                   ($report_type === 'by_grade' ? 'Grade ' . $row['grade_level'] : $row['class']) ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                                                <?= number_format($row['count']) ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                                                <?= $total_students > 0 ? number_format(($row['count'] / $total_students) * 100, 1) : 0 ?>%
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
    ...
    <script>
        ...
        document.getElementById('grade_level').addEventListener('change', function() {
            ...
            if (gradeLevel) {
                ...
                sections.forEach(function(section) {
                    ...
                    option.textContent = 'Grade ' + gradeLevel + ' - ' + section;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            <?php if ($report_type === 'summary'): ?>
                ...
                new Chart(gradeCtx, {
                    type: 'bar',
                    data: {
                        labels: <?= json_encode(array_map(function($g) { return 'Grade ' . $g['grade_level']; }, $report_data['by_grade'])) ?>,
                        datasets: [{
                            label: 'Number of Students',
                            data: <?= json_encode(array_column($report_data['by_grade'], 'count')) ?>,
            ...
            <?php else: ?>
                ...
                new Chart(ctx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            legend: {
                                position: '<?= $report_type === 'by_gender' ? 'right' : 'top' ?>',
                                labels: { boxWidth: 12 }
                            },
                            title: {
                                display: true,
        ...
        function exportToExcel() {
                ...
                <?php foreach ($report_data['by_grade'] as $grade): ?>
                    html += `
                        <tr>
                            ...
                            <td><?= $grade['count'] ?></td>
                            <td><?= $total_students > 0 ? number_format(($grade['count'] / $total_students) * 100, 1) : 0 ?>%</td>
                        </tr>
                    `;
                <?php endforeach; ?>
            ...
            <?php else: ?>
                html += `
                    <tr>
                        <th><?= $report_type === 'by_gender' ? 'Gender' : ($report_type === 'by_grade' ? 'Grade Level' : 'Class') ?></th>
                        <th>Number of Students</th>
                        <th>Percentage</th>
                    </tr>
                `;
                ...
                <?php foreach ($report_data as $row): ?>
                    html += `
                        <tr>
                            <td><?= $report_type === 'by_gender' ? ($row['gender'] ?: 'Not Specified') : 
                                   ($report_type === 'by_grade' ? 'Grade ' . $row['grade_level'] : $row['class']) ?></td>
                            <td><?= $row['count'] ?></td>
                            <td><?= $total_students > 0 ? number_format(($row['count'] / $total_students) * 100, 1) : 0 ?>%</td>
                        </tr>
                    `;
                <?php endforeach; ?>
            ...
            a.href = url;
            a.download = 'enrollment_report_<?= $report_type ?>_<?= date('Y-m-d') ?>.xls';
